...
var signaturesSpec = {
    required: NO,
    validator: function sigSpecValidator(signatureArray) {
        var isValid = YES, errors = [], unspecifiedProps = [], result;

        for (var i = 0; i < signatureArray.length; i++) {
            this.push("[" + i + "]");
            result = objConformToSpec(signatureArray[i], docSpec);
            this.pop();
...
var nameSpec = {
    ...
    validator: function(name){
        var isValid = YES, errors = [];
        if(!is(name).aString()) {
            errors.push("name can only be a string");
        }

        return { isValid: isValid, errors: errors, unspecifiedProperties: [] };
    }
};
